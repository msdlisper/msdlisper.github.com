...
单测的目的之一, 就在于此: 快速回归, 保证兼容性. 其实后面讲的[黑盒测试](#_4), 也是这个目的
...
黑盒测试的痛点在于数据, 想一想:我们qa同学为了测一个case, 可能要[花5分钟造一个数据](#qa)(反正我是至少要花10分钟), 然后测出前端一个低级bug, qa同学, 心情可能就不那么美丽了... 
...
这些状态可以通过url注入到前端代码, 也可以通过window.zpdebugCase来注入(*随便起的名字, 但注意不要和其他全局变量冲突*), 最后在http.js里 注入到每个请求的header里(代码见下面); 最后, yapi通过header将这个接口对应状态的数据返回, 见 [高级mock](#yapi-mock)
...
- 通过url()来打开测试网页, 并通过 [参数](#url) 开启调试, 指定用户为未签约未绑卡状态
    + 访问mock数据, 根据debugCase=0n1n 返回未签约未绑卡应该返回的数据
- 判断, 页面正常加载出来了
- 模拟用户点了一下同意协议
- 模拟用户点了一下一键开通
- 给window注入zpdebugCase, 让下一次[请求](#httpjs)的head的debugCase='0b1s'
